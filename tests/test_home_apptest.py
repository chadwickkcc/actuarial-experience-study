...
def test_home_renders_workflow_and_links():
    ...
    assert any("Experience Study Tool" in t.value for t in at.title)
    subs = [s.value for s in at.subheader]
    assert any("How it fits together" in s for s in subs)         # workflow diagram section
    assert any("A guided tour" in s for s in subs)                # storyline section
    # The two explainer cards are present.
    md = " ".join(m.value for m in at.markdown)
    assert "Where the AI fits" in md
