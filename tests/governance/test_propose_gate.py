"""UAT 4.1b — the `propose` permission is enforced on assumption Steps 1-2.
...
def test_every_step2_write_button_is_propose_gated():
    """Source guard: every Step-2 write action carries the propose gate.

    Step 2 has two write buttons (Save as PROPOSED, Submit for sign-off). Each must
    render with ``disabled=... not _can_propose ...`` and re-check
    ``require(_user, Action.PROPOSE)`` in its handler.
    """
    import pathlib

    src = pathlib.Path("ui/views/21_assumption_step2.py").read_text(encoding="utf-8")
    for label in (
        "💾 Save as PROPOSED",
        "Submit for sign-off →",
    ):
        assert label in src, f"Step-2 button missing: {label}"
    # One gate + one server-side re-check per write button (>= 2 of each).
    assert src.count("not _can_propose") >= 2, "a Step-2 write button lost its disabled gate"
    assert src.count("require(_user, Action.PROPOSE)") >= 2, "a Step-2 write handler lost its server-side re-check"
...
def test_step1_create_enabled_for_analyst():
    analyst = User(
    ...
    with patch("src.governance.auth.current_user", return_value=analyst):
        at = AppTest.from_file("ui/views/20_assumption_step1.py", default_timeout=60)
        at.run()
    assert not at.exception, f"Step 1 raised on render: {list(at.exception)}"
...
@pytest.mark.skipif(not DB_PATH.exists(), reason="local experience_study.duckdb not present")
def test_step1_create_disabled_for_non_proposer():
    chief = User(
